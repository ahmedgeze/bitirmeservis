...
def deneme():
    ...
    try:
        ...
        # pickle_twelve_control(0,9,5,len(data['result']),hash_coin_twelve,'twelve_pickle1',data)
        ...
        #
        # with open('btc_pickle','wb') as btc:
        #     eth_p=pickle.load(btc)
        #
        # with open('eth_pickle','wb') as handle:
        #     eth_p=pickle.load(eth)
...
def pickle_twelve_control(hour,minute,seconds,length,picklefile,picklefile_name,datas):
    if hour==0 and minute<10 and (seconds>1 or seconds<10):
        count=0
...
def price_dbsave(datas,pickle_file,starting_time):
    ...
    for x in range(0,len(datas['result']),1):
        ...
        if str(datas['result'][x]['Summary']['MarketName'][0:3]) == 'BTC':
            ...
            #     "kur_id__kur_name":"BTC",
            #
            # }
            # array_btc.append(btc_dict.copy())
            bc+=1
        ...
        if str(datas['result'][x]['Summary']['MarketName'][0:3]) == 'ETH':
            ...
            #     "high":datas['result'][x]['Summary']['High'],
            #     "last":datas['result'][x]['Summary']['Last'],
            #     "kur_id__kur_name":"ETH",
            #
            # }
            # array_eth.append(eth_dict.copy())
            et+=1
            ...
            #     "base_volume":datas['result'][x]['Summary']['BaseVolume'],
            #     "time":starting_time,
            #     "koin_id__koin_name":usd_coinname,
            #     "change":((datas['result'][x]['Summary']['Last']-hash_dictionary[datas['result'][x]['Summary']['MarketName']])*100)/datas['result'][x]['Summary']['Last'],
            #     "volume":datas['result'][x]['Summary']['Volume'],
            #     "low":datas['result'][x]['Summary']['Low'],
            #     "high":datas['result'][x]['Summary']['High'],
            #     "last":datas['result'][x]['Summary']['Last'],
            #     "kur_id__kur_name":"USDT",
            #
            # }
            # array_usd.append(usdt_dict.copy())
    ...
    # with open("pickle_btc", 'wb') as btc_p:
    #      pickle.dump(list(array_btc),btc_p)
    #
    # with open("pickle_eth", 'wb') as eth_p:
    #      pickle.dump(list(array_eth),eth_p)
    #
    # with open("pickle_usdt", 'wb') as usd_p:
    #      pickle.dump(list(array_btc),usd_p)
    #
    # with open('pickle_usdt','rb') as read:
    #     pickle1=pickle.load(read)
    #
    # file1=open('file1.txt','w')
    # file1.write(str(pickle1))
    # file1.close()
